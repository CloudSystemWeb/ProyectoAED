...
public class CodigoCliente extends JDialog implements ActionListener, MouseListener {
	...
	public CodigoCliente() {
		...
		{
			label = new JLabel("");
			label.setIcon(new ImageIcon(CodigoCliente.class.getResource("/imagenes/the-desktop-hd-Windows-7-Black-wallpapers (1).jpeg")));
	...
	void listar(){
		if(c.tamaño()>0){
			for(int i=0; i<c.tamaño(); i++){
				...
				Object datos [] = {clien.getCodCliente(),clien.getNombreCliente(),clien.getApellidoCliente(),clien.getDirrecion(),clien.getFechaNacimiento(),clien.getFechaAfiliacion(),clien.getEstadoCivil(),
				clien.getTelefono(),clien.getDNI(),clien.getUsuario(),clien.getContraseña()};
				dtm.addRow(datos);
				}
		}
		else{
	...
	protected void do_btnSalir_actionPerformed(ActionEvent arg0) {
		...
		Reserva.txtCodigoCliente.setText(dato);
		this.dispose();
		}catch(Exception e){
			this.dispose();
		}
	...
	void seleccion(){
		if(cboOpcion.getSelectedIndex()==0){
			txtNombre.setEditable(false); txtCodigo.setEditable(true);btnBuscar.setEnabled(true); btnBuscar_1.setEnabled(false);
			txtCodigo.setText(""); txtNombre.setText("");txtCodigo.requestFocus();
		}
		else{
			txtNombre.setEditable(true); txtCodigo.setEditable(false);btnBuscar.setEnabled(false);btnBuscar_1.setEnabled(true);
			txtCodigo.setText(""); txtNombre.setText("");txtNombre.requestFocus();
		}
	}
	//---------------------------------------
	String leerNombre(){
